...
fn would_group_be_captured(&self, group_row: usize, group_col: usize, group_stone: Stone, 
new_stone_row: usize, new_stone_col: usize, new_stone: Stone) -> bool {
...
if self.board[nr][nc] == Stone::Empty && !(nr == new_stone_row && nc == new_stone_col) {
return false;
}
}
}
...
fn would_friendly_group_have_liberties(&self, group_row: usize, group_col: usize, 
group_stone: Stone, new_row: usize, new_col: usize) -> bool {
let group = self.get_group(group_row, group_col, group_stone);
for &(r, c) in &group {
for (nr, nc) in self.get_neighbors(r, c) {
...
ui.separator();
ui.label(format!("Captured - Black: {}, White: {}", 
...
let pos = top_left + egui::Vec2::new(col as f32 * CELL_SIZE, row as f32 * CELL_SIZE);
